...
public final class JwwParser {
    ...
    public func parse() throws -> JwwDrawing {
        ...
        //   -8        DWORD   グループ状態: 0=非表示 1=表示のみ 2=編集可 3=書込グループ
        //   -4        DWORD   書込レイヤ
        //   +0        double  縮尺の分母(1/50なら50.0)
        //   +8        DWORD   プロテクト
        //   +12+k*8   DWORD   レイヤk(0〜15)の状態(同じコード)、+16+k*8 プロテクト
        ...
        if let nameR = readCStr(12) {
            ...
            for gi in 0..<16 {
                ...
                if sOff + 148 <= len {
                    ...
                    // グループ状態はブロック先頭(縮尺の8バイト手前)。以前の +140 は次グループの値を
                    // 読んでいた(1グループずれ)。M8.1で実図面4件のヘッダ全体を歩いて確認
                    let gv = u32(sOff - 8)
